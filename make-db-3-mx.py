    ...
    with open(filename, "r") as ins:
        for line in ins:
            try:
                ...
                T_value=False##################porque se hace esto?????
            except:
                try:
                    #Fixing GOES T
        ...
        try:
            ...
            altitude = get_altitude(LAT, LON, t_max) #esta linea podria ser get_altitude(LAT,LON, t_max) 
            azimuth = get_azimuth(LAT,LON,t_max) #esta linea podria ser get_altitude(LAT,LON, t_max) si tienes la ultima version de pysolar
	    	# la mia es la 0.6
            ...
            try:
                ...
                peak_flux=0.0
            if peak_flux > 0.0 and altitude >= 0:
                numofevents += 1
